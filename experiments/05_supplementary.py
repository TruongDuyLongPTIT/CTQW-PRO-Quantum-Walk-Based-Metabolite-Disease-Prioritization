...
from config import (RESULTS_DIR, CACHE_DIR, T_CTQW, NH_GAMMA, RECON3D_CURRENCY_METABOLITE)
...
def run_s1_experiment(ctx):
    ...
    ctqw_fn = lambda seeds: run_ctqw(seeds, T_CTQW)
    ...
    for gamma in GAMMA_GRID_S1:
        ...
        run_nh = make_nh_pro(
            ...
            RECON3D_CURRENCY_METABOLITE, pro_nodes, float(gamma), T_CTQW)
...
# ════════════════════════════════════════════════════════════════
T_GRID_S2 = [0.05, 0.1, 0.2, 0.5, 0.8]  # grid dùng để chọn T_CTQW trong paper (Mục 4.x)
