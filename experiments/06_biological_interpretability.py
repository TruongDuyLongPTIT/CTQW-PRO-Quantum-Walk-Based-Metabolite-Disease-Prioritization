...
Output:
  ...
  2. Top-20 predictions với HMDB ID và PubMed search link
...
def pubmed_link(name, disease):
    """Generate PubMed search URL for metabolite + disease."""
    q = f"{name} {disease}".replace(" ", "+")
...
# Cofactor node indices in G_cc (exclude from predictions)
...
# BUILD NH-CTQW-PRO
# ══════════════════════════════════════════════════════════════════
print(f'\nBuilding NH-CTQW-PRO (γ={NH_GAMMA}, t={T_FIXED})...')
t0 = time.time()
run_nh = make_nh_pro(
...
for disease_key, disease_name in DISEASES.items():
    ...
    for i, s in enumerate(valid_seeds, 1):
        ...
        hlink = hmdb_link(hmdb)
        print(f"  {i:>3}. {sname:<50}  {hmdb:>13}  {hlink}")
    ...
    print(f"  {'Rank':>4}  {'Name':<50}  {'HMDB':>13}  {'Score':>10}  PubMed search")
    print('  ' + '-'*110)

    for i, c in enumerate(top, 1):
        plink = pubmed_link(c['name'], disease_name)
        hlink = hmdb_link(c['hmdb_id'])
        print(f"  {i:>4}. {c['name']:<50}  {c['hmdb_id']:>13}  "
              f"{c['score']:>10.6f}  {plink}")
        ...
        all_rows.append({
            ...
            'score':           c['score'],
            'pubmed_search':   pubmed_link(c['name'], disease_name),
    ...
    print(f'\n  → Verify each prediction at the PubMed links above.')
    print(f'    Fill tier/pmid/direct_quote columns in the output CSV.')
...
print('\n' + '='*65)
print(f'Saved: {out}')
print(f'Total predictions: {len(df)} ({len(DISEASES)} diseases × {TOP_K})')
print()
print('NEXT STEPS for literature validation:')
print('  1. Open biological_interpretability.csv')
print('  2. For each row: click pubmed_search link')
print('  3. Find paper measuring this metabolite in this disease')
print('  4. Fill columns:')
print('       tier         → ESTABLISHED / POTENTIAL / NOVEL')
print('       pmid         → e.g. 25612837')
print('       direct_quote → exact sentence from paper')
print('       notes        → any additional context')
print('='*65)
print('Done.')