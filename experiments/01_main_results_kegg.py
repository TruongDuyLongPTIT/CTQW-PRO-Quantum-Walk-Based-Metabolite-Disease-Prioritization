...
from config import CACHE_DIR, T_FIXED, NH_GAMMA_KEGG, KEGG_CURRENCY_METABOLITE
...
print('='*60); print('STEP 1 — Build graph (KEGG)')

G_cc, node_idx, N, A_cc, pathway_mets = build_kegg_metabolism_data()
...
Apro_eigvals, Apro_eigvecs = compute_eigendecomp(
    A_pro, CACHE_DIR / 'gpro_kegg_eigdecomp.npz')
Acc_eigvals, Acc_eigvecs   = compute_eigendecomp(
    A_cc, CACHE_DIR / 'gcc_kegg_eigdecomp.npz')
print('  Done.')

# ----------------TABLE 1 — RWR vs CTQW on G_cc ----------------
print('\n' + '='*60)
print('[Table 1] RWR vs CTQW on G_cc (KEGG)...')
...
print('\n[Table 2] PROFANCY vs CTQW-PRO on G_pro (KEGG)...')
...
# ----------TABLE 3 — NH-CTQW-PRO--------------------------------
print('\n[Table 3] NH-CTQW-PRO (KEGG)...')

print(f'  Building NH eigendecomp (γ={NH_GAMMA_KEGG})...', end=' ', flush=True)
t0_nh = time.time()
run_nh = make_nh_pro(
    A_pro, idx_pro, N, N_PRO, _pro_src, _pro_dst,
    KEGG_CURRENCY_METABOLITE, pro_nodes, NH_GAMMA_KEGG, T_FIXED)
...
print('\n' + '='*72)
print('TABLE 1: RWR vs MetaboRank-lite vs CTQW (G_cc, KEGG)')
_print_full_and_dedup(all_t1, ['RWR', 'MetaboRank-lite', 'CTQW'])

print('\n' + '='*72)
print('TABLE 2: PROFANCY vs MetaboRank-lite-PRO vs CTQW-PRO (G_pro, KEGG)')
_print_full_and_dedup(all_t2, ['PROFANCY', 'MetaboRank-lite-PRO', 't=0.1'])

print('\n' + '='*72)
print('TABLE 3: CTQW-PRO vs NH-CTQW-PRO (G_pro, KEGG)')
_print_full_and_dedup(all_t3, ['CTQW-PRO', f'NH γ={NH_GAMMA_KEGG}'])
...
for label in EVAL_SETS:
    df_c  = all_t2[label].get('t=0.1')
    df_nh = all_t3[label].get(f'NH γ={NH_GAMMA_KEGG}')
    if df_c is not None and df_nh is not None:
        wilcoxon_table(df_nh, df_c, label,
                       method_a=f'NH γ={NH_GAMMA_KEGG}', method_b='CTQW-PRO')
...
_ci_sources = [
    ...
    (f'NH γ={NH_GAMMA_KEGG}', lambda l: all_t3[l].get(f'NH γ={NH_GAMMA_KEGG}')),
]
