import numpy as np
from graph import compute_coreness
from config import T_CTQW, RWR_R, RWR_TOL, RWR_MAXITER

# RWR (G_cc) — lõi dùng chung cho PROFANCY và các baseline trên G_pro
...
def make_ctqw_gcc(eigvals, eigvecs, node_idx, N):
    ...
    def run_ctqw_gcc(seed_nodes, t=T_CTQW, _n=_N):
...
def make_profancy(P_pro, idx_pro, node_idx, N, N_PRO, r=RWR_R):
    ...
    _N = N
    # ánh xạ G_pro -> node_idx, tính một lần (tương đương vòng lặp qua node_idx)
    _src = np.array([idx_pro[nd] for nd in node_idx if nd in idx_pro], dtype=np.intp)
    _dst = np.array([node_idx[nd] for nd in node_idx if nd in idx_pro], dtype=np.intp)
...
# MetaboRank-LITE (G_pro): PPR / PR toàn cục (Frainay et al., 2019)
def make_metaborank_lite(P_pro, idx_pro, node_idx, N, N_PRO, r=RWR_R):
    ...
    def run_metaborank_lite(seed_nodes, _n=_N):
...
def make_ctqw_pro(eigvals, eigvecs, idx_pro, N, N_PRO, _pro_src, _pro_dst):
    ...
    def run_ctqw_pro(seed_nodes, t=T_CTQW, _n=_N):
...
def make_nh_pro(A_pro, idx_pro, N, N_PRO, _pro_src, _pro_dst,
                CURRENCY_METABOLITE, pro_nodes, gamma, t=T_CTQW):
