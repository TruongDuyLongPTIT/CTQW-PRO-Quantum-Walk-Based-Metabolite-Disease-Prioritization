...
def wilcoxon_table(df_a, df_b, label,
    ...
    if df_a is None or df_b is None:
        print(f'  {label}: thieu ket qua -- bo qua')
        return None
    ...
    # 2 phương pháp trùng khớp tuyệt đối -> scipy.wilcoxon không kiểm định được
    if np.allclose(da.loc[shared, metrics].values, db.loc[shared, metrics].values):
    ...
    print(f"  {'Metric':<8} {method_b:>16} {method_a:>16} "
          f"{'Delta':>8} {'Median':>8} {'p_bonf':>10}  Sig")
    ...
    for met in metrics:
        ...
        med   = float(np.median(a - b))   # trung vị hiệu theo bệnh: chiều thắng của bệnh "điển hình"
        try:
            _, p = _scipy_stats.wilcoxon(a, b, alternative='two-sided', zero_method='wilcox')
        except ValueError:          # vd. mọi hiệu bằng 0 ở riêng chỉ số này
            p = np.nan
